...
import (
	...
	"github.com/lasthyphen/dijets-cli/pkg/utils"
	...
	anrutils "github.com/lasthyphen/dijets-network-runner/utils"
...
func (d *LocalDeployer) doDeploy(chain string, chainGenesis []byte, genesisPath string) (ids.ID, ids.ID, error) {
	dijetsNodeBinPath, err := d.SetupLocalEnv()
	if err != nil {
		return ids.Empty, ids.Empty, err
	}
	...
	if !networkBooted {
		if err := d.startNetwork(ctx, cli, dijetsNodeBinPath, runDir); err != nil {
			return ids.Empty, ids.Empty, err
		}
	}
...
// * checks if dijetsnode is installed in the local binary path
// * if not, it downloads it and installs it (os - and archive dependent)
// * returns the location of the dijetsnode path
func (d *LocalDeployer) SetupLocalEnv() (string, error) {
	...
	dijetsNodeBinPath := filepath.Join(avagoDir, "dijetsnode")
	...
	exists, err = storage.FileExists(dijetsNodeBinPath)
	if !exists || err != nil {
		return "", fmt.Errorf(
			"evaluated dijetsNodeBinPath to be %s but it does not exist", dijetsNodeBinPath)
	}

	return dijetsNodeBinPath, nil
}
