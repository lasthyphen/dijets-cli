...
func TestDeployToLocal(t *testing.T) {
	...
	binDir := filepath.Join(app.GetAvalanchegoBinDir(), "dijetsnode-"+avagoVersion)
	...
	// create a dummy dijetsnode file, deploy will check it exists
	f, err := os.Create(filepath.Join(binDir, "dijetsnode"))
	require.NoError(err)
	defer func() {
		_ = f.Close()
	}()
