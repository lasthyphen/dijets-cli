...
var (
	// path to dijetsnode config file
	avagoConfigPath string
	// path to dijetsnode plugin dir
	pluginDir string
...
you provide the --dijetsnode-config flag, this command attempts to edit the config file
at that path.
...
This command currently only supports Subnets deployed on the Fuji Testnet and Mainnet.`,
	...
	cmd.Flags().StringVar(&avagoConfigPath, "dijetsnode-config", "", "file path of the dijetsnode config file")
	cmd.Flags().StringVar(&pluginDir, "plugin-dir", "", "file path of dijetsnode's plugin directory")
...
func joinCmd(_ *cobra.Command, args []string) error {
	if printManual && (avagoConfigPath != "" || pluginDir != "") {
		return errors.New("--print cannot be used with --dijetsnode-config or --plugin-dir")
	}
	...
	if !skipWhitelistCheck {
		...
		if !forceWhitelistCheck {
			ask := "Would you like to check if your node is allowed to join this subnet?\n" +
				"If not, the subnet's control key holder must call dijets subnet\n" +
...
but until the node is whitelisted, it will not be able to validate this subnet.`)
	...
	if avagoConfigPath == "" && pluginDir == "" {
		...
		choice, err := app.Prompt.CaptureList(
			"How would you like to update the dijetsnode config?",
	...
	if pluginDir == "" {
		...
		if pluginDir == "" {
			pluginDir, err = app.Prompt.CaptureString("Path to your dijetsnode plugin dir (likely dijetsnode/build/plugins)")
			if err != nil {
				return err
			}
		}
...
func isNodeValidatingSubnet(subnetID ids.ID, network models.Network) (bool, error) {
	...
	if nodeIDStr == "" {
		...
		ux.Logger.PrintToUser("Check https://dev.dijets.io for instructions about how to query the NodeID from your node")
...
If you installed dijetsnode with the install script, your plugin directory is likely
~/.dijetsnode/build/plugins.
...
For example:
./build/dijetsnode --network-id=%s --whitelisted-subnets=%s

If you start the node via a JSON config file, add this to your config file:
whitelisted-subnets: %s

TIP: Try this command with the --dijetsnode-config flag pointing to your config file,
