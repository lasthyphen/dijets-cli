...
import (
	...
	"github.com/lasthyphen/dijets-cli/pkg/key"
	"github.com/lasthyphen/dijets-cli/pkg/localnetworkinterface"
	"github.com/lasthyphen/dijets-cli/pkg/models"
	"github.com/lasthyphen/dijets-cli/pkg/prompts"
	"github.com/lasthyphen/dijets-cli/pkg/subnet"
	"github.com/lasthyphen/dijets-cli/pkg/txutils"
	"github.com/lasthyphen/dijets-cli/pkg/ux"
	"github.com/lasthyphen/dijets-cli/pkg/vm"
	...
	ledger "github.com/lasthyphen/dijetsnode/utils/crypto/ledger"
	...
	spacesvmchain "github.com/lasthyphen/spacesvm/chain"
...
Dijets-CLI only supports deploying an individual Subnet once per network. Subsequent
...
dijets network clean to reset all deployed chain state. Subsequent local deploys
...
so you can take your locally tested Subnet and deploy it on Fuji or Mainnet.`,
	...
	cmd.Flags().StringVar(&userProvidedAvagoVersion, "dijetsnode-version", "latest", "use this version of dijetsnode (ex: v1.17.12)")
...
func PrintReadyToSignMsg(
	...
	ux.Logger.PrintToUser("  dijets transaction commit %s --input-tx-filepath %s", chain, outputTxPath)
}
...
func PrintRemainingToSignMsg(
	...
	ux.Logger.PrintToUser("  dijets transaction sign %s --input-tx-filepath %s", chain, outputTxPath)
}
	...
	if networkRunning {
		if userProvidedAvagoVersion == "latest" {
			if runningRPCVersion != configuredRPCVersion && !skipRPCCheck {
				return "", fmt.Errorf(
					"the current dijetsnode deployment uses rpc version %d but your subnet has version %d and is not compatible",
					runningRPCVersion,
			...
			return "", errors.New("incompatible dijetsnode version selected")
		}
