...
```sh
curl -sSfL https://raw.githubusercontent.com/lasthyphen/dijets-cli/main/scripts/install.sh | sh -s
```
...
```
curl -sSfL https://raw.githubusercontent.com/lasthyphen/dijets-cli/main/scripts/install.sh | sh -s -- -b <relative directory>
```
...
```bash
dijets subnet create <subnetName>
dijets subnet deploy <subnetName>
```
...
You can provide a global node config to edit the way your local dijetsnode nodes perform under the hood. To provide such a config, you need to create an dijets-cli config file. By default, a config file is read in from $HOME/.dijets-cli.json. If none exists, no error will occur. To provide a config from a custom location, run any command with the flag `--config <pathToConfig>`.
...
To build Dijets-CLI, you'll first need to install golang. Follow the instructions here: https://go.dev/doc/install.
...
The binary will be called `./bin/dijets`.

### Docker

To make Dijets CLI work in a docker container, add this
...
- `~/.dijets-cli/snapshot` will contain all saved snapshots, which can for example be used to pass work around
