...
import (
	...
	spacesvmclient "github.com/lasthyphen/spacesvm/client"
...
func CheckAvalancheGoExists(version string) bool {
	avagoPath := path.Join(GetBaseDir(), constants.AvalancheCliBinDir, constants.AvalancheGoInstallDir, "dijetsnode-"+version)
...
func GetWhilelistedSubnetsFromConfigFile(configFile string) (string, error) {
	...
	if err != nil && !errors.Is(err, os.ErrNotExist) {
		return "", fmt.Errorf("failed to load dijetsnode config file %s: %w", configFile, err)
	}
