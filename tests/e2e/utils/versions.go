...
// The idea is to finally calculate which VM is compatible with which dijetsnode,
...
// GetLatestAvagoByProtoVersion returns the latest dijetsnode version which
...
// GetAvagoURL returns the compatibility URL for dijetsnode
...
// GetVersionMapping returns a map of specific VMs resp. dijetsnode e2e context keys
...
func GetVersionMapping(mapper VersionMapper) (map[string]string, error) {
	...
	// now get the dijetsnode compatibility object
	...
	// evaluate two dijetsnode versions which are consecutive
	// and run with the same RPC version.
	// This is required for the for the "can deploy with multiple dijetsnode versions" test
	...
	// and then a compatible dijetsnode
	//
	// To avoid having to iterate again, we'll also fill the values
	// for the **latest** compatible dijetsnode and Subnet-EVM
	for i, ver := range subnetEVMversions {
		...
		// there exists at least one compatible dijetsnode.
	...
	// this is simpler, we just need the latest and its compatible dijetsnode
	spacesVMversions, spacesVMmapping, err := getVersions(mapper, models.SpacesVM)
	if err != nil {
		return nil, err
	}
	// the assumption is that the latest SpacesVM ALWAYS has a compatible dijetsnode already
...
// getAvagoCompatibility returns the compatibility for dijetsnode
